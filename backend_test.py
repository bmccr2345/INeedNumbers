...
def main_ai_coach_v2_tests():
    ...
    def test_pdf_generation_functionality(self):
        ...
        print(f"\n📄 PDF GENERATION FUNCTIONALITY TESTING SUMMARY:")
        print(f"   ✅ Successful tests: {successful_tests}/{total_tests}")
        print(f"   📈 Success rate: {(successful_tests/total_tests)*100:.1f}%")
        
        if overall_success:
            print("   🎉 PDF Generation Functionality - TESTING COMPLETED SUCCESSFULLY")
        else:
            print("   ❌ PDF Generation Functionality - CRITICAL ISSUES FOUND")
            
        return overall_success, results
    
    def test_pdf_generation_authentication(self):
        """Test that PDF generation requires proper authentication"""
        print("\n🔐 TESTING PDF GENERATION AUTHENTICATION...")
        
        try:
            # Test without authentication
            print("   🔍 Testing PDF generation without authentication...")
            response = requests.post(
                f"{self.base_url}/api/brand/test-pdf",
                timeout=15
            )
            ...
            if response.status_code == 401:
                print("   ✅ PDF generation properly requires authentication (401)")
                return True, {"unauthenticated_blocked": True, "status": response.status_code}
            else:
                print(f"   ❌ PDF generation should require authentication, got {response.status_code}")
                return False, {"error": "Authentication not enforced", "status": response.status_code}
                
        except Exception as e:
            print(f"   ❌ Error testing PDF generation authentication: {e}")
            return False, {"error": str(e)}
    
    def test_pdf_generation_with_auth(self):
        """Test PDF generation with authenticated user"""
        print("\n📄 TESTING PDF GENERATION WITH AUTHENTICATION...")
        ...
        try:
            ...
            print("   🔍 Logging in for PDF generation test...")
            login_response = session.post(
                f"{self.base_url}/api/auth/login",
                json=login_data,
                timeout=15
            ...
            print("   ✅ Login successful for PDF generation test")
    ...
    def test_pdf_content_validation(self):
        ...
        try:
            ...
            if login_response.status_code != 200:
                print(f"   ❌ Login failed for content validation: {login_response.status_code}")
                return False, {"error": "Login failed"}
            ...
            if pdf_response.status_code == 200:
                print("   ✅ PDF generated successfully for content validation")
                ...
                if pdf_size < 5000:  # Should be substantial for a formatted document
                    ...
                    print(f"   ✅ PDF size indicates substantial content: {pdf_size} bytes")
                ...
                if successful_validations >= 3:  # Allow one failure
                    print(f"   ✅ PDF content validation passed ({successful_validations}/4 checks)")
                    return True, validation_results
                else:
                    print(f"   ❌ PDF content validation failed ({successful_validations}/4 checks)")
                    return False, validation_results
            else:
                print(f"   ❌ PDF generation failed for content validation: {pdf_response.status_code}")
                return False, {"error": "PDF generation failed", "status": pdf_response.status_code}
...
def main_pdf_generation_tests():
    ...
    if success_rate >= 80:
        print("\n🎉 PDF GENERATION: EXCELLENT - All critical functionality working")
