...
# Rate limiting middleware (uses Redis in production)
...
async def upload_brand_asset(
    ...
    try:
        ...
        if not file.content_type or file.content_type not in ALLOWED_MIME:
            allowed_types = ", ".join(ALLOWED_MIME)
            raise HTTPException(
                status_code=400, 
        ...
        max_size = ASSET_MAX_MB * 1024 * 1024  # Convert MB to bytes
        if len(file_data) > max_size:
            raise HTTPException(
                status_code=400, 
                detail=f"File too large. Maximum size is {ASSET_MAX_MB}MB"
            )
        ...
        # Upload to S3
        url = await upload_to_s3(processed_data, key, "image/png")
...
async def get_pnl_deals(
    ...
    """Get all deals for a specific month"""
    try:
...
async def api_health_check():
    """
    API Health check endpoint - mirrors main health endpoint
    Returns application status, version, and service health.
    """
    redis_health = redis_client.health_check()
